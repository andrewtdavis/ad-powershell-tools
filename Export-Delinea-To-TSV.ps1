<#
.SYNOPSIS
    Export Delinea (Centrify) zone/group information and users’ Unix attributes to TSV files,
    resolving the backing AD groups on demand.

.DESCRIPTION
    This script enumerates Delinea zones (optionally limited to a subtree), enumerates the zone’s
    group profiles, maps them back to AD group names, pulls AD members on demand (cross-domain,
    recursive, cycle-safe), and for each AD user tries to read the Delinea user profile in that zone.

    Output:
      - delinea-<zone>-groups.tsv          : groups in the zone + their GIDs
      - delinea-<zone>-<adgroup>-users.tsv : users in that zone for that AD group, with Unix info

    A per-zone, per-identifier cache is used to avoid repeated Delinea lookups. You can persist it.
...
.PARAMETER SearchBase
    OU path (slash/backslash form) in that AD domain where groups live.
    Converted to an LDAP DN.
...
if ($Log) {
    try {
        ...
        $LogWriter.WriteLine("Script    : Export-Delinea-To-TSV.ps1")
...
try {
    ...
    function Get-AdGroupMembersOnDemand {
        ...
        if (-not $g) {
            Write-Log "  (on-demand) AD group '$GroupName' not found in any domain."
        ...
        try {
            ...
            Write-Log "  (on-demand) AD group '$GroupName' was found but members could not be read."
            $AdMembersByGroup[$GroupName] = @()
            return @()
        }
        ...
        Write-Log "  (on-demand) pulled AD group '$GroupName' with $($members.Count) members from '$groupDomain'."
        return $members
    }

    Write-Log "Starting Delinea export using group pattern '$GroupPattern'."
    ...
    try {
        ...
        Write-Log "Delinea module loaded."
    } catch {
    ...
    if ($LoadCache -and (Test-Path $CachePath)) {
        try {
            ...
            if ($loaded -is [hashtable]) {
                ...
                Write-Log "Loaded Delinea cache from ${CachePath} (entries: $($DelineaUserCache.Count))"
            }
        } catch {
    ...
    if ($CdmRootZonePath) {
        ...
        if ($roots) {
            $AllZones = $AllZones | Where-Object { $_.CanonicalName -like "*$first*" }
            Write-Log "Limiting to Delinea subtree '$CdmRootZonePath' ($($AllZones.Count) zone(s))."
        } else {
            Write-Warning "Could not resolve Delinea path '$CdmRootZonePath'; using all zones."
        }
    } else {
        Write-Log "Will process $($AllZones.Count) Delinea zones."
    }
    ...
    foreach ($zone in $AllZones) {
        ...
        if (-not (Should-SkipFileVerbose -Path $zoneGroupFile -Full:$Full -Reason "zone group list exists")) {
            ...
            Write-Log "  Wrote $zoneGroupFile"
        }

        $zoneGroups2 = Get-CdmGroupProfile -Zone $zone -ErrorAction SilentlyContinue
        if (-not $zoneGroups2) {
            Write-Log "  No Delinea group profiles found in this zone."
            continue
        }
        ...
        foreach ($gp in $zoneGroups2) {
            ...
            if (-not $ProcessAllDelineaGroups) {
                if ($adName -notlike $GroupPattern) {
                    Write-Log "  Skipping zone group '$($gp.Name)' (AD group '$adName' does not match '$GroupPattern')."
                    continue
                }
            }
            ...
            if (-not $adMembers -or $adMembers.Count -eq 0) {
                Write-Log "  Skipping zone group '$($gp.Name)' (AD group '$adName' has no members)."
                continue
            }
            ...
            foreach ($usr in $adMembers) {
                ...
                foreach ($cand in $candidates) {
                    ...
                    if ($cachedVal) {
                        if ($cachedVal -is [pscustomobject] -and $cachedVal.PSObject.Properties.Name -contains 'NotFound') {
                            Write-Log "    [$idx/$tot] cache hit (NOT FOUND) for '$($usr.SamAccountName)'"
                            $cached = $true
                            $prof   = $null
                            break
                        } else {
                            Write-Log "    [$idx/$tot] cache hit for '$($usr.SamAccountName)'"
                            $cached = $true
                            $prof   = $cachedVal
                            break
                        }
                ...
                if (-not $cached) {
                    Write-Log "    [$idx/$tot] resolving '$($usr.SamAccountName)'"
                    foreach ($cand in $candidates) {
            ...
            Write-Log "  Wrote $zoneGroupUsersFile"
            ...
            Write-Log "  Cache entries: $($DelineaUserCache.Count)"
        }
    }

    Write-Log "Delinea export done."
}
finally {
    Close-Log
}