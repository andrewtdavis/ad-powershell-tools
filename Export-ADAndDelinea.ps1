 <#
.SYNOPSIS
    Export Active Directory group membership and Delinea (Centrify) zone/group Unix attributes
    to per-group TSV files, with cross-domain AD expansion and per-zone Delinea caching.

.DESCRIPTION
    This script automates extraction of Windows and Unix identity mappings from
    hybrid AD + Delinea environments. It can:
      • Recursively expand AD groups across multiple domains and forests
      • Enumerate Delinea zones and their mapped AD groups
      • Export both AD and Delinea membership data to TSV
      • Cache Delinea lookups to avoid redundant user profile queries
      • Resume work from cached data for large or long-running exports

    Typical use cases:
      – Auditing AD group and Unix mapping consistency
      – Comparing zone membership between environments
      – Bulk export for reporting or compliance review

.PARAMETER GroupPattern
    AD group name pattern (supports wildcards), e.g. "APP_*" or "LINUX-ADMINS*".
    Used to select both AD groups and corresponding Delinea groups.

.PARAMETER SearchDomain
    DNS domain of the primary AD source, e.g. "example.corp.local".
    The script will start group enumeration here and traverse trusted domains as needed.

.PARAMETER SearchBase
    Organizational Unit (OU) path—using slashes or backslashes—
    defining where to search for AD groups, e.g. "Infrastructure/Groups".
    Automatically converted to an LDAP DN.

.PARAMETER OutputDir
    Destination directory for all TSV exports. Created if it does not exist.

.PARAMETER CdmRootZonePath
    Optional Delinea zone path (e.g. "Global Zone/Engineering").
    Restricts enumeration to that zone subtree.

.PARAMETER SkipAD
    Skip the AD export phase; only run the Delinea phase.

.PARAMETER Full
    Overwrite any existing TSVs instead of skipping recently-generated ones.

.PARAMETER ProcessAllDelineaGroups
    Process all Delinea zone groups regardless of the -GroupPattern filter.

.PARAMETER LoadCache
    Load a previously saved Delinea lookup cache to reuse prior results.

.PARAMETER WriteCache
    Save the current Delinea cache to disk after each zone is processed.

.PARAMETER CachePath
    Optional file path for the serialized cache (CLIXML format).
    Defaults to "<OutputDir>\delinea-user-cache.clixml".

.EXAMPLES
    # Example 1 – Full AD + Delinea export
    ...
    # Example 2 – Delinea-only run (skip AD)
    .\Export-AdAndDelinea.ps1 `
        -SkipAD `
        -GroupPattern "LINUX-*" `
        -SearchDomain "example.corp.local" `
    ...
    # Example 3 – Delinea run with persistent cache
    .\Export-AdAndDelinea.ps1 `
        -SkipAD `
        -GroupPattern "SERVICE_*" `
        -SearchDomain "example.corp.local" `
        -SearchBase "IT" `
        -OutputDir "exports" `
    ...
    # Example 4 – Force overwrite of all TSVs
    .\Export-AdAndDelinea.ps1 ... -Full

.NOTES
    • AD recursion is cycle-safe; nested groups and circular references are handled gracefully.
    • Cross-domain user resolution is based on the DC components of each object’s DN.
    • Delinea lookups are cached per zone and per identifier (sAMAccountName/UPN).
    • “UID NOT FOUND” indicates that the user has no Unix profile in that zone.
    • Output files are tab-separated to preserve special characters.
